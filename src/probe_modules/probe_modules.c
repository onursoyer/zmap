...
void fs_add_ip_fields(fieldset_t *fs, struct ip *ip) {
	...
	// [MOBI]
	fs_add_string(fs, "header_length", ip->ip_hl);
	fs_add_string(fs, "ip_v", ip->ip_v);
	fs_add_string(fs, "ip_tos", ip->ip_tos);
	fs_add_string(fs, "ip_len", ip->ip_len);
	fs_add_string(fs, "ip_p", ip->ip_p);
	fs_add_string(fs, "ip_sum", ip->ip_sum);
}
